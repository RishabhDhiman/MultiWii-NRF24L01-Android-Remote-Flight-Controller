...
#if defined(FAILSAFE) && !defined(PROMICRO)
  if (GoodPulses == (1 << THROTTLEPIN) + (1 << YAWPIN) + (1 << ROLLPIN) + (1 << PITCHPIN))
  { // If all main four chanells have good pulses, clear FailSafe counter
    GoodPulses = 0;
    if (failsafeCnt > 20)
      failsafeCnt -= 20;
    else
...
#if defined(RCAUXPIN8)
  if (!(pin & 1 << 0))
  { // indicates if the bit 0 of the arduino port [B0-B7] is not at a high state (so that we match here only descending PPM pulse)
#endif
#if defined(RCAUXPIN12)
    if (!(pin & 1 << 4))
    { // indicates if the bit 4 of the arduino port [B0-B7] is not at a high state (so that we match here only descending PPM pulse)
#endif
  ...
  ISR(INT6_vect)
  {
    ...
    if (!(PINE & (1 << 6)))
    {
      ...
      if (900 < diff && diff < 2200)
      {
        ...
        { // if Throttle value is higher than FAILSAFE_DETECT_TRESHOLD
          if (failsafeCnt > 20)
            failsafeCnt -= 20;
          else
  ...
  void rxInt(void)
  {
    ...
    else
    {
      ...
      { // Only if the signal is between these values it is valid, otherwise the failsafe counter should move up
        ...
        if (GoodPulses == 0x0F)
        { // If first four chanells have good pulses, clear FailSafe counter
          GoodPulses = 0;
          if (failsafeCnt > 20)
            failsafeCnt -= 20;
          else
...
/**************************************************************************************/
  ...
  void readSerial_RX(void)
  {
    ...
    while (SerialAvailable(RX_SERIAL_PORT) > SPEK_FRAME_SIZE)
    ...
    { // The interrupt handler saw at least one valid frame start since we were last here.
      if (SerialAvailable(RX_SERIAL_PORT) == SPEK_FRAME_SIZE)
      { // A complete frame? If not, we'll catch it next time we are called.
        ...
        if (failsafeCnt > 20)
          ...
          failsafeCnt = 0; // Valid frame, clear FailSafe counter
#endif
      }
      else
...
//############# FREQUENCY HOPPING FUNCTIONS #################
  ...
  void Read_OpenLRS_RC()
  {
    ...
    if ((currentTime - last_hopping_time > 25000))
    { // automatic hopping for clear channel when rf link down for 25ms.
      Red_LED_ON;
    ...
    if (nIRQ_0)
    { // RFM22B INT pin Enabled by received Data
      Red_LED_ON;
  ...
  void spekBind()
  {
    ...
    while (1)
    { // Do not return.  User presses reset button to return to normal.
      blinkLED(4, 255, 1);
